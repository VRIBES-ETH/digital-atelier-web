...
export default function AdminDashboard() {
    ...
    const handleProcess = async () => {
        ...
        try {
            // Import dynamically or use server action directly
            const { generateOnboardingAnalysis } = await import('@/app/actions/onboarding');

            const response = await generateOnboardingAnalysis(transcript, clientName, clientPhase);

            if (response.success && response.data) {
                setResult(response.data);
            } else {
                setError(response.error || "Error desconocido en el proceso.");
            }
        } catch (err: any) {
